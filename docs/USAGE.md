...
By default, vitals and trackers use keys that include the full path of the current note (for example `din_health::Characters/Marlowe`), so each note’s pools are independent.
